...
- Package: `CodexBar-Windows-0.25-win-x64.zip`
- Checksum: `CodexBar-Windows-0.25-win-x64.zip.sha256`
...
5. Smoke launch the packaged app from a clean folder and confirm the tray icon, taskbar dock, settings window, and enabled providers open without crashing.
...
4. Confirm the release contains both Windows assets:
...
- Portable zip only; installer and automatic updates are planned for a later release.
...
1. Download the release zip from GitHub on a Windows 11 machine.
...
3. Unzip and run `CodexBar.WinApp.exe`.
4. Open a smoke-test issue if any provider fails with normal local credentials.
